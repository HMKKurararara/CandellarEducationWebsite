...
export const content = {
  ...
  posts: () => getContent<Card[]>(`*[_type=='post']|order(coalesce(publishedAt,_updatedAt) desc){_id,title,'slug':slug.current,excerpt,'image':coverImage}`, demoPosts),
  videos: () => getContent<Card[]>(`*[_type=='video']|order(coalesce(publishedAt,_updatedAt) desc){_id,title,'slug':slug.current,summary,'image':thumbnail,'url':coalesce(videoFile.asset->url,videoUrl)}`, demoVideos),
